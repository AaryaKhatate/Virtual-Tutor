...
export default function UploadBox() {
  ...
  return (
    <div className="relative">
      {!selectedFile ? (
        <div
          className={`spotlight relative mx-auto max-w-3xl rounded-2xl border-2 border-dashed px-6 py-14 sm:py-16 md:py-20 transition-all duration-300 ${isDragging ? 'border-indigo-400/80 bg-indigo-950/30 shadow-glow' : 'border-slate-600/40 bg-slate-900/40 hover:border-indigo-400/50 hover:bg-slate-900/60 hover:shadow-glow2'}`}
          onMouseMove={(e) => {
        ...
        >
          <div className="flex flex-col items-center text-center">
            <div className={`mb-4 h-14 w-14 rounded-full flex items-center justify-center bg-indigo-500/10 text-indigo-300 ${isDragging ? 'animate-float' : ''}`}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M12 16v-8M8 12l4-4 4 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M20 16.5a3.5 3.5 0 01-3.5 3.5h-9A3.5 3.5 0 014 16.5c0-1.7 1.2-3.1 2.8-3.4.3-2.7 2.6-4.8 5.2-4.8 2.6 0 4.9 2.1 5.2 4.8 1.6.3 2.8 1.7 2.8 3.4z" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
            </div>
            <h3 className="text-lg sm:text-xl font-medium text-white">Drag & Drop your PDF here</h3>
            ...
            <div className="mt-4">
              ...
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="opacity-90"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                Select PDF
              </button>
              <input ref={inputRef} type="file" accept="application/pdf,.pdf" className="hidden" onChange={onChange} />
            </div>

            <p className="mt-3 text-xs text-slate-500">Supported: PDF up to 50MB</p>
          </div>
        </div>
      ) : (
